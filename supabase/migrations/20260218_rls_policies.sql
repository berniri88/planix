...
using ( public.check_trip_membership(trip_id, auth.uid()) );
...
using ( 
  exists (
    select 1 from itinerary_versions v 
    where v.id = itinerary_items.version_id 
    and public.check_trip_membership(v.trip_id, auth.uid())
  )
);
...
using (
  exists (
    select 1 from trips where id = participants.trip_id and owner_id = auth.uid()
  )
);
